...
class Storage:
    ...
    window.stor.ListPhotoStor       # for faster loading of ani/vid
    fulllevel.stor.ListPhotoStor    # for faster loading of ani/vid
    
        File:
        Ani:
        .ListPhotoStor[window.fileindex][PhotoImage]
    
    fulllevel.photoalbum.PreviewStor      # for preview of photoalbum
    fulllevel.manga.PreviewStor           # for preview of manga
    
        File:
        Img:
        .PreviewStor[window.fileindex][Image]
    ...
    def __init__(self, name):
        ...
        self.ListStor = []       # window.stor.ListStor or fulllevel.stor.ListStor or fulllevel.photoalbum.ListStor or fulllevel.manga.ListStor
        self.ListPhotoStor = []   # window.stor.ListPhotoStor or fulllevel.stor.ListPhotoStor
        self.PreviewStor = []     # fulllevel.photoalbum.PreviewStor or fulllevel.manga.PreviewStor
        self.backend_run = False
        self.backend_run_photoimage = False
        self.backend_run_preview = False
    ...
    def img_storage(self, listindex):
        ...
        try:
            ...
            print("Auto storage (img) unable to process - %i", listindex)
            logging.info("Auto storage (img) unable to process - %i", listindex)
    ...
    def ani_storage(self, listindex):
        ...
        try:
            ...
            print("Auto storage (ani) unable to process - %i", listindex)
            logging.info("Auto storage (ani) unable to process - %i", listindex)
    ...
    def vid_storage(self, listindex):
        ...
        try:
            ...
            print("Auto storage (vid) unable to process - %i", listindex)
            logging.info("Auto storage (vid) unable to process - %i", listindex)
    ...
    def clear_one_storage(self, fileindex):
        ...
        self.PreviewStor[fileindex] = None
    ...
    def stop_storage(self, is_delete):
        ...
        if self.backend_run:
            ...
            try:
                self.thread_storage.join(1)
                self.thread_storage_preview.join(1)
    ...
    def create_reset_storage(self):
        ...
        self.PreviewStor = [None] * len(window.fullfilelist)
        self.last_index = None
    ...
    def clear_preview_storage_all(self):
    ...
    def photoimage_storage(self, listindex):
        ...
        try:
            ...
            print("Auto storage (PhotoImage) unable to process - %i", listindex)
            logging.info("Auto storage (PhotoImage) unable to process - %i", listindex)
        
    def backend_preview_storage(self, fileindex):
        ...
        try:
            gc.disable()                                   # garbage collection in thread may cause instability
            self.backend_run_preview = True                        # running thread
            storage_range = min(self.FORWARD_STORAGE + 1, len(window.fullfilelist))
            print("start backend_preview_storage :", self.name)
            logging.info("start backend_preview_storage: %s", self.name)
            ...
            for i in range(storage_range):
                ...
                if self.PreviewStor[listindex] is None:      # if not yet cached
                    
                    filepath = window.fullfilelist[listindex]
                    file_ext = os.path.splitext(filepath)[1].lower()
                    
                    if file_ext in window.supported_img:
                        self.preview_storage(listindex)

            print("Auto Preview storage done: ", self.name)         # finish running loop
            logging.info("Auto Preview storage done: %s", self.name)

            self.backend_run_preview = False
            
        except StopIteration:                  # kill thread
            print("Auto Preview storage stopped: ", self.name)
            logging.info("Auto Preview storage stopped: %s", self.name)
            self.backend_run_preview = False 
    ...
    def preview_storage(self, listindex):
        ...
        try:
            ...
            self.PreviewStor[listindex] = resize_img    # cache
            print("Preview Cached: ", self.name , " - ", listindex)
            logging.info("Preview Cached: %s - %s", self.name, listindex)
                    
        except StopIteration:                  # kill thread
            print("Preview storage stopped: ", self.name)
            logging.info("Preview storage stopped: %s", self.name)
            ...
            print("Auto storage (Preview) unable to process - %i", listindex)
            logging.info("Auto storage (Preview) unable to process - %i", listindex)
...
class WindowGUI(tk.Frame):
    ...
    def append_filelist(self, add_filelist, random_process = False):
        ...
        if not random_process:                   # avoid dataframe reset when clicking random button
            self.dataframe_reset(True)
        
        if window.is_web_mode:
            self.added_folder = os.path.split(add_filelist[0])[0]
            self.added_files = add_filelist
            # print(self.added_folder)
    ...
    def remove_filelist(self, drop_filelist):
        print("remove_filelist")
        logging.info("remove_filelist") 
        
        i = 0
        for path in drop_filelist:
            i = i + 1
            remove_index = window.fullfilelist.index(path)
            window.fullfilelist.remove(path)
            
            listlevel.listbox1.delete(remove_index)
        
        print("Delete %i items" %i)
        logging.info("Delete %i items" %i)
    ...
    def filelist_to_go(self, direct_list):
        ...
        if window.is_full_mode and not window.is_stop_ani:     # for ANI in full mode
            ...
            if window.is_photoalbum_mode:          # photo album mode
                ...
                fulllevel.photoalbum.thread_storage_preview = threading.Thread(target = fulllevel.photoalbum.backend_preview_storage, args = (window.fileindex,))
                
                fulllevel.photoalbum.stop_backend = False
                fulllevel.photoalbum.thread_storage.start()  
                fulllevel.photoalbum.thread_storage_preview.start()  
...
# In[Webscraping]
    ...
    def enter_web_mode(self, http_path):
        ...
        elif http_path.find("hbmanga.com") or http_path.find("czmanga.com") >= 0:
            self.web = "baozimh"
            self.import_cookies()
    ...
    def import_cookies(self):
        ...
        exe_dir = os.path.split(sys.argv[0])[0]        
        config_path = os.path.join(exe_dir, "cookies.ini")
        config.read(config_path)
        
        if self.web == "exhentai":
            try:
                self.url_cookies = eval(config.get("ImageViewer", "exhentai"))
            
            except:
                self.url_cookies = {"igneous":"8rzrv0yjz9kpew1bstv", "sk":"ngk0ktce7hft83dem4njd9tzyt6y", "arp_scroll_position":"208.8000030517578", "ipb_pass_hash":"a9a5869db2c244a2294a38aeafa65178", "ipb_member_id":"528230"}

        elif self.web == "baozimh":
            try:
                self.url_cookies = eval(config.get("ImageViewer", "baozimh"))
            ...
            try:
                self.url_cookies = eval(config.get("ImageViewer", "cartoonmad"))
    ...
    def quit_web_mode(self):
        ...
        if window.is_manga_mode:
            fulllevel.manga.thread_storage = threading.Thread(target = self.manga.backend_storage, args = (window.fileindex,))
            fulllevel.manga.thread_storage_preview = threading.Thread(target = self.manga.backend_preview_storage, args = (window.fileindex,))
            
            fulllevel.manga.stop_backend = False
            fulllevel.manga.thread_storage.start()  
            fulllevel.manga.thread_storage_preview.start()  
    ...
    def batch_download(self, next_path, url_cookies, to_dir, from_page, download_pages):
        ...
        for i in range(from_page, to_page):
            ...
            except IndexError:       # raise error if i = self.total_page
                pass
        ...
        if self.skip_download:
            ...
            # remove files from window.fullfilelist
            latest_folder_files = os.listdir(self.web_output_folder)
            
            drop_filelist = []
            for file_path in self.added_files:
                file_name = os.path.split(file_path)[1]
                
                if file_name not in latest_folder_files:
                    drop_filelist.append(file_path)

            window.remove_filelist(drop_filelist)
...
# Exhentai
    def enter_exhentai_webscraping(self, http_path):
        ...
        if "?" in http_path:                   # fix the issue of fail to run when not on page 1
            skip_len = http_path.find("?")
            http_path = http_path[:skip_len]
        ...
        if self.first_format:      # fix the issue when 1st pic is not jpg
            savefilepath[0] = os.path.splitext(savefilepath[0])[0] + self.first_format
...
# In[Quit Button]
    ...
    def quit(self):
        ...
        try:
            ...
            del fulllevel.stor.PreviewStor
...
# In[Forward Button]
    ...
    def forward(self, slide_forward = False, event=None):                                  # all modes except manga mode
        ...
        if self.fileindex == len(self.fullfilelist):  # back to start
            if window.is_web_mode:                     # fix the bug of web mode back to start occasionally 
        ...
        # check cache
            ...
            try:    # when not yet entered photoalbum mode
                if fulllevel.photoalbum.PreviewStor[self.fileindex] is not None:
                    is_previewcached = "photoalbum"  
            ...
            if self.is_full_mode:
                ...
                if self.is_photoalbum_mode:
                    ...
                    # if fulllevel.photo_preview_visible and not window.process_enhance:
                    if fulllevel.photo_preview_visible:
                        window.ShowIMG_Preview(nextpath, False, False, is_previewcached)
        ...
        if file_ext in self.supported_img:                 # img
            ...
            if self.is_full_mode:
                ...
                if self.is_photoalbum_mode:
                    ...
                    # if fulllevel.photo_preview_visible and not window.process_enhance:
                    if fulllevel.photo_preview_visible:
                        window.ShowIMG_Preview(filepath, False, False, is_previewcached)
...
# In[Show Image]
    ...
    def ShowIMG(self, pic_path, is_cached = False):                         # window mode
        ...
        if window.next_enhance and window.process_enhance:
            resize_img = window.ShowIMG_ImageEnhance()
            self.image = ImageTk.PhotoImage(image = resize_img)
    ...
    def ShowIMG_Full(self, pic_path, is_fullcached = False, is_vid = False, slide_forward = False):                     # full screen mode
        ...
        if window.next_enhance and not is_vid and window.process_enhance:
            resize_img = window.ShowIMG_ImageEnhance()
            fulllevel.image = ImageTk.PhotoImage(image = resize_img)
    ...
    def ShowIMG_FadeOut(self, pre_pic, next_pic):
        ...
        if window.next_enhance and window.process_enhance:
            pre_pic_enhance = window.ShowIMG_ImageEnhance(True, pre_pic)
            next_pic_enhance = window.ShowIMG_ImageEnhance(True, next_pic)

            pre_pic = np.array(pre_pic_enhance)      # convert PIL image to cv2 format
            next_pic = np.array(next_pic_enhance)    # convert PIL image to cv2 format
    ...
    def ShowIMG_FullPhotoalbum(self, pic_path, initial=False, is_photocached=False):          # Photo Album mode
        ...
        if is_photocached:
            ...
            if window.full_w / fulllevel.img_w >= window.full_h / fulllevel.img_h:  
                ...
                if window.next_enhance and window.process_enhance:
                    image_IE = window.ShowIMG_ImageEnhance()
                    window.Apply_ImageEnhance(image_IE, initial)
    ...
    def ShowIMG_ImageEnhance(self, enhance = True, image_input = None):
        ...
        if image_input is None:
            ...
            image_IE = Image.fromarray(np.uint8(image_input))         
    ...
    def Apply_ImageEnhance(self, image_IE, initial = False):
        # print("Apply_ImageEnhance")
        # logging.info("Apply_ImageEnhance")
        ...
        if not window.is_full_mode:           # window mode
            ...
            if not window.is_photoalbum_mode and not window.is_manga_mode:            # simple full mode / slide mode
                ...
                if not initial:
                    ...
                    window.process_enhance = True     # to make sure preview get enhance for 1st adjustment
                    window.ShowIMG_Preview(pic_path, False, False)       # apply enhance to preview
                
        del image_IE
    
    def Check_ImageEnhance(self):
        
        # to avoid unnecessary image enhance
        if round(window.brightness, 2) == 1.0 and round(window.contrast, 2) == 1.0 and round(window.sharpness, 2) == 1.0 and round(window.color, 2) == 1.0:
            window.process_enhance = False
        
        else:
            window.process_enhance = True
    ...
    def brightness_decrease(self, event=None):
        ...
        image_IE = window.ShowIMG_ImageEnhance()
        window.Apply_ImageEnhance(image_IE, False)
        window.Check_ImageEnhance()
    ...
    def ShowIMG_Preview(self, pic_path, is_vid, listbox, is_cached = False):                     # full screen mode
        ...
        if not is_cached:
            ...
            if is_cached == "photoalbum":
                print("preview cached ***")
                resize_img = fulllevel.photoalbum.PreviewStor[window.fileindex]
                h, w = resize_img.shape[:2]
            
            else:
                print("preview cached ***")
                resize_img = fulllevel.manga.PreviewStor[window.fileindex]
                h, w = resize_img.shape[:2]
        ...
        # try:      # fix TypeError: Cannot handle this data type: (1, 1, 3), <u2
            ...
            if window.next_enhance and window.process_enhance:
                resize_img = window.ShowIMG_ImageEnhance(True, resize_img)
                fulllevel.photo_canvas.image = ImageTk.PhotoImage(image = resize_img)
...
# In[When FocusIn]
    ...
    def event_focusin(self, event):
        ...
        if self.allow_event_focusin:
            print("event_focusin")
            logging.info("event_focusin")
            
            self.allow_event_focusin = False
            ...
            window.after(1000, self.control_focusin)
            
    def control_focusin(self):
        
        # to prevent calling event_focusin very frequently in fulllevel.motion
        self.allow_event_focusin = True
...
class FullscreenGUI(WindowGUI):
    ...
    def fullscreen_create(self):
        ...
        self.popup.bind("<FocusIn>", window.event_focusin)
        ...
        self.enhancer_button4 = ttk.Button(self.manga_con, text = "Enhance", style='primary.TButton', command = settinglevel.enhancer_show, width=8)
        self.enhancer_button4.pack(side=tk.LEFT,padx= 10)
    ...
    def photo_preview_event_resize(self, event):
        ...
        if window.is_photoalbum_mode:
            fulllevel.photoalbum.clear_preview_storage_all()
            
            self.photoalbum.thread_storage_preview = threading.Thread(target = self.photoalbum.backend_preview_storage, args = (window.fileindex,))
            self.photoalbum.thread_storage_preview.start()  
        
        if window.is_manga_mode:
            fulllevel.manga.clear_preview_storage_all()
...
# In[Manga Mode]
    ...
    def start_manga(self, event=None):
        ...
        self.manga.thread_storage_preview = threading.Thread(target = self.manga.backend_preview_storage, args = (window.fileindex,))
        
        self.manga.stop_backend = False
        self.manga.thread_storage.start()  
        self.manga.thread_storage_preview.start()  
    ...
    def show_manga_top(self):
        ...
        if self.manga.ListStor[window.pre_mangaindex] is not None:
            ...
            if window.next_enhance and window.process_enhance:
                resize_img = window.ShowIMG_ImageEnhance(True, resize_img)
                globals()['image_%s' % window.pre_mangaindex] = ImageTk.PhotoImage(image = resize_img)
            ...
            try:
                ...
                print("unable to open %s" %pic_path)
                logging.info("unable to open %s" %pic_path)
                
                pic_path = window.ico_path
                img = Image.open(pic_path)
    ...
    def show_manga_mid(self):
        ...
        globals()['image_%s' % window.fileindex] = ImageTk.PhotoImage(image = resize_img)
        ...
        if self.manga.PreviewStor[window.fileindex] is None:
            is_previewcached = False
        
        else:
            is_previewcached = "manga"
        
        if self.photo_preview_visible:
            window.after(0, window.ShowIMG_Preview, pic_path, False, False, is_previewcached)
    ...
    def show_manga_bot(self):
        ...
        if self.manga.ListStor[window.next_mangaindex] is not None:
            ...
            if window.next_enhance and window.process_enhance:
                resize_img = window.ShowIMG_ImageEnhance(True, resize_img)
                globals()['image_%s' % window.next_mangaindex] = ImageTk.PhotoImage(image = resize_img)
...
class ListboxGUI():
    ...
    def select_photo_mode(self, filepath, file_ext):
        ...
        if fulllevel.photoalbum.PreviewStor[window.fileindex] is not None:
            is_previewcached = "photoalbum"
        else:
            is_previewcached = False
    ...
    def select_manga_mode(self):
        ...
        fulllevel.manga.thread_storage_preview = threading.Thread(target = fulllevel.manga.backend_preview_storage, args = (window.fileindex,))
...
class SettingGUI(WindowGUI):
    ...
    def enhancer_click(self,event=None):
        ...
        if event is None:
            if enhance_current:
                image_IE = window.ShowIMG_ImageEnhance(True)
                
            else:
                image_IE = window.ShowIMG_ImageEnhance(False)
        ...
        window.Apply_ImageEnhance(image_IE)
    ...
    def enhancer_show(self):
        ...
        try:
            ...
            fulllevel.enhancer_button4.config(text = "Hide", command=self.enhancer_hide)
    ...
    def enhancer_hide(self):                            # in windw and fullscreen mode
        ...
        try:
            ...
            fulllevel.enhancer_button4.config(text = "Enhance", command=settinglevel.enhancer_show)
...
bug fix:
photo album mode -> select list -> preview can't update occasionally (can't replicate)
