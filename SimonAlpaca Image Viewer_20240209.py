...
class WindowGUI(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.filter_con = tk.Toplevel(self.parent)
    ...
    def browse_file(self):
        ...
        file_name = filedialog.askopenfilename(filetypes = (("Image files", list(self.supported_img) + list(self.supported_ani) + list(self.supported_vid)),("List", list(self.supported_list)), ("All files", "*")))
   
        if file_name != "":
            if not os.path.splitext(file_name)[1] in (self.supported_img | self.supported_ani | self.supported_vid | self.supported_list):
            ...
            if os.path.splitext(file_name)[1] in self.supported_list:
                self.import_list(file_name)
                
            else:
                window.go(browsefilelist)
...
r''' 
functionality:
customise list (add/remove files/folder)
manga reader (exhentai, dm5)
browse folder
