        ...
        # check cache
            ...
            if self.is_full_mode:
                ...
                if self.is_photoalbum_mode:
                    self.ShowIMG_FullPhotoalbum(nextpath, True)  # Photo Album mode
                    self.ShowIMG_Preview(nextpath, False, False)
                    window.after(20, self.ShowIMG_FullPhotoalbum, nextpath)  # # slight delay for next slide to fix crash issue of photo album mode
                else:   
    ...
    def backward(self, event=None):                               # all modes except manga mode
        ...
        if file_ext in self.supported_img:                 # img
            ...
            if self.is_full_mode:
                ...
                if self.is_photoalbum_mode:
                    self.ShowIMG_FullPhotoalbum(filepath, True)  # Photo Album mode
                    self.ShowIMG_Preview(filepath, False, False)
                    # self.ShowIMG_FullPhotoalbum(filepath)   # Photo Album mode
                    window.after(20, self.ShowIMG_FullPhotoalbum, filepath)   # Photo Album mode
...
# In[Show Image]
    ...
    def ShowIMG_FullPhotoalbum(self, pic_path, initial=False):          # Photo Album mode
        ...
        if window.full_w / fulllevel.img_w >= window.full_h / fulllevel.img_h:                    # if the image is narrower than screen
            ...
            while window.is_slide_check and not initial:
        ...
        if window.is_slide_check and not initial and not window.stop_album:
            if fulllevel.slide_timer >= settinglevel.default_timer + 2 and window.is_slide_check: # backward
                fulllevel.fullbackward()
    ...
    def ShowANI(self, pic_path, is_cached = False, fileindex = None):                       # window mode
        ...
        while not window.is_stop_ani and not window.is_full_mode:          # loop gif until stop
            ...
            try:            
                ...
                self.pic_canvas.delete("all")                 # remove existing image
                image = pic_frame[i]
                self.pic_canvas.create_image(pic_w / 2, pic_h / 2, anchor="center", image = image)  # create image frame by frame
            
            except Exception:   # fix the issue of some gifs doesn't have last frame
                print("Exception: No frame ", i)
                logging.info("Exception: No frame ", i)
    ...
    def ShowANI_Full(self, pic_path, is_fullcached = False, fileindex = None):                       # window mode
        ...
        while not window.is_stop_ani and window.is_full_mode:          # loop gif until stop
            ...
            try:
                ...
                image = pic_frame[i]
                fulllevel.pic_canvas.delete("all")                 # remove existing image
                fulllevel.pic_canvas.create_image(window.full_w / 2, window.full_h / 2,anchor="center",image = image)  # create image frame by frame
...
class FullscreenGUI(WindowGUI):
    ...
    def enter_full(self):
        ...
        self.photo_con.geometry("+%d+%d" % (window.full_w / 2 - 650/2 + self.mon_x, window.full_h - 64 + self.mon_y))
        self.photo_preview.geometry("+%d+%d" % (50 + self.mon_x, window.full_h - 250 + self.mon_y))
        self.manga_con.geometry("+%d+%d" % (window.full_w / 2- 650 /2 + self.mon_x, window.full_h - 64 + self.mon_y))
    ...
    def fullscreen_create(self):
        ...
        self.full_con.bind("<a>", fulllevel.start_photoalbum)
        self.full_con.bind("<m>", fulllevel.start_manga)
        ...
        self.photo_con.geometry("780x50+%d+%d" % (window.full_w / 2 - 780/2, window.full_h - 64))
...
# In[Photo Album Mode]
    ...
    def zoomout_photoalbum(self, event=None):
        ...
        if not window.event_zoom_ongoing:         # can't run until the previous zoom has been finished
            ...
            window.photoalbum_zoom_factor = round(max(0.7, window.photoalbum_zoom_factor - 0.1), 2)
...
class SettingGUI():
    ...
    def setting_create(self):                          # in window and fullscreen mode
        ...
        self.parent.resizable(False,False)
        self.parent.overrideredirect(1)
        self.parent.geometry('350x730+%d+%d' %(int(self.setting_x), int(self.setting_y)))    # re adjust list position
