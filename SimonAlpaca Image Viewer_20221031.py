...
class Storage:
    ...
    fulllevel.manga.ListStor
        ...
        .ListStor[window.fileindex] #Img or Ani 
            ...
            .ListStor[window.fileindex]{"gif_duration" : [gif_duration of each frame], "is_cached" : True / False, "Image" : [Image of each gif frame]}
    
    Lists:
    window.storage_size
    # 1 storage_size = 1 image with size of full screen
    
        window.storage_size[0] = [storage size of each file of window.stor.ListStor]
        window.storage_size[1] = [storage size of each file of fulllevel.stor.ListStor]
        window.storage_size[2] = [storage size of each file of fulllevel.manga.ListStor]
    ...
    def write_ani_storage(self, fileindex, gif_duration = None, img = None, cached = None):
        ...
        if img is not None:
            try:
                ...
                print("AttributeError: 'NoneType' object has no attribute 'append'")
                print("Listindex : ", fileindex)
    ...
    def img_storage(self, listindex):
        ...
        try:
            ...
            if img is not None:
                ...
                if self.name == "window":
                    window.storage_size[0][listindex] = total_screen
                
                elif self.name == "fulllevel":
                    window.storage_size[1][listindex] = total_screen
                    
                elif self.name == "manga":
                    window.storage_size[2][listindex] = total_screen
        ...
        try:
            ...
            while success:
                ...
                if success:
                    ...
                    try:
                        self.ListStor[listindex].append(image)
    ...
    def clear_one_storage(self, fileindex):
        ...
        if self.name == "window":
            window.storage_size[0][fileindex] = 0
            
        elif self.name == "fulllevel":
            window.storage_size[1][fileindex] = 0
            
        elif self.name == "manga":
            window.storage_size[2][fileindex] = 0
    ...
    def create_reset_storage(self):
        ...
        if self.name == "window":
            window.storage_size[0] = [0] * len(window.fullfilelist)
            
        elif self.name == "fulllevel":
            window.storage_size[1] = [0] * len(window.fullfilelist)
            
        elif self.name == "manga":
            window.storage_size[2] = [0] * len(window.fullfilelist)
...
# In[Control Storage]
    ...
    def reset_storage_size(self):
        ...
        window.storage_size = [0] * 3                      # [0] = window.stor; [1] = fulllevel.stor; [2] = manga.stor
        window.storage_size[0] = [0] * len(window.fullfilelist)
        window.storage_size[1] = [0] * len(window.fullfilelist)
        window.storage_size[2] = [0] * len(window.fullfilelist)
    ...
    def cal_storage_size(self):
        ...
        return round(sum(window.storage_size[0]) + sum(window.storage_size[1]) + sum(window.storage_size[2]),2)
...
# In[Go Button]
    ...
    def nat_sort(self, file):
        ...
        if not file_ext in (self.supported_img | self.supported_ani | self.supported_vid):
    ...
    def forward(self, event=None):                                  # all modes except manga mode
        ...
        # check cache
            ...
            if self.is_full_mode:
                ...
                window.after(20, self.ShowANI_Full, nextpath, is_fullcached)      # slight delay for ANI for releasing memory
                ...
                window.after(10, self.ShowANI, nextpath, is_cached)      # slight delay for ANI for releasing memory
    ...
    def backward(self, event=None):                               # all modes except manga mode
        ...
        if file_ext in self.supported_img:                 # img
            ...
            if self.is_full_mode:                  
                ...
                is_vid = True
                window.after(20, self.ShowANI_Full, filepath, is_fullcached)      # slight delay for ANI for releasing memory   
                ...
                window.after(10, self.ShowANI, filepath, is_cached)      # slight delay for ANI for releasing memory
...
# In[Show Image]
    ...
    def ShowIMG_Full(self, pic_path, is_fullcached = False, is_vid = False):                     # full screen mode
        print("ShowIMG_Full")
        ...
        if not is_fullcached:
            ...
            if window.full_w > 1 and window.full_h > 1:
                ...
                if window.rotate_degree == 0 and window.zoom_factor == 1 and not is_vid:
    ...
    def ShowIMG_List(self, pic_path, is_vid):                     # full screen mode
        print("ShowIMG_List")
        logging.info("ShowIMG_List")    
...
# In[Show Animation]
    ...
    def ShowANI(self, pic_path, is_cached = False):                       # window mode
        ...
        if is_cached:
            ...
            if os.path.splitext(pic_path)[1] in window.supported_ani:
                gif_len, gif_duration, gif_frame = self.Gif_Pic(pic_path, pic_w, pic_h)
            
            elif os.path.splitext(pic_path)[1] in window.supported_vid:
        ...
        while not window.is_stop_ani and not window.is_full_mode:          # loop gif until stop
            ...
            except IndexError:   # fix the issue of some gifs doesn't have last frame
                pass
            ...
            #print("time diff: ", time_diff)
            sleep_time = max(0.01, gif_duration[i]/1000 * gif_speed  - (time_diff + 0.0))
            #print("sleep : ", sleep_time)
            sleep_time = min(1, sleep_time)
    ...
    def Vid_Pic(self, pic_path, pic_w, pic_h):
        ...
        # print(gif_duration)
        if not self.event_resize_stopstorage:       # stop storage after event_resize
            self.stor.write_ani_storage(window.fileindex, gif_duration)   # empty duration
        ...
        while success:
            ...
            if success:
                ...
                if not self.event_resize_stopstorage:       # stop storage after event_resize
                    self.stor.write_ani_storage(window.fileindex, None, image)
...
# Show Animation in Fullscreen, Slide mode and Photo Album mode
    ...
    def Gif_Pic_Full(self, pic_path):
        ...
        fulllevel.stor.write_ani_storage(window.fileindex, gif_duration)
        ...
        for img in cv_gif:
            img = cv2.resize(img, w_h, interpolation = cv2.INTER_AREA)
            gif_frame.append(img)
            fulllevel.stor.write_ani_storage(window.fileindex, None, img)
            
        fulllevel.stor.write_ani_storage(window.fileindex, None, None, True)
    ...
    def Vid_Pic_Full(self, pic_path):
        ...
        while success:
            ...
            if success:
                ...
                image = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
                gif_frame.append(image)
                fulllevel.stor.write_ani_storage(window.fileindex, None, image)
...
class FullscreenGUI(WindowGUI):
    ...
    def motion(self, event=None):                      # in full screen
        ...
        if y < window.full_h - 70:
            ...
            window.thread_hidecursor1 = window.after(50, self.hide_cursor_precheck, event.x, event.y)    # hide cursor
...
class ListboxGUI():
    ...
    def create_listbox(self):
        ...
        self.parent.geometry('296x550+%d+%d' %(int(settinglevel.listbox_x), int(settinglevel.listbox_y)))    # re adjust list position
        ...
        self.list_canva_h = 190
        self.list_canva.resizable(False,False)
        self.list_canva.overrideredirect(1)
        self.list_canva.geometry('%dx%d+0+0' %(self.list_canva_w, self.list_canva_h))    
    ...
    def list_canva_show(self, event=None):
        ...
        window.ShowIMG_List(filepath, is_vid)
...
class SettingGUI():
    ...
    def getconfig(self, column, var):
        ...
        try:
            ...
            elif self.config.get("ImageViewer", str(column)) == "True":
    ...
    def setting_create(self):                          # in window and fullscreen mode
        ...
        self.parent.resizable(False,False)
        self.parent.overrideredirect(1)
        self.parent.geometry('350x700+%d+%d' %(int(self.setting_x), int(self.setting_y)))    # re adjust list position
